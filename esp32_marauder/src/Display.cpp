...
/*
void setVerticalScrollArea(uint16_t topFixedArea, uint16_t bottomFixedArea) {
    uint16_t scrollHeight = tft.height() - topFixedArea - bottomFixedArea;

    tft.startWrite();
    tft.writecommand(0x33); // VSCRDEF command for ST7796S
    tft.writedata(topFixedArea >> 8);
    tft.writedata(topFixedArea & 0xFF);
    tft.writedata(scrollHeight >> 8);
    tft.writedata(scrollHeight & 0xFF);
    tft.writedata(bottomFixedArea >> 8);
    tft.writedata(bottomFixedArea & 0xFF);
    tft.endWrite();
}*/
...
/*void Display::scrollAddress(uint16_t vsp) {
  #ifdef HAS_ST7796
    uint16_t topFixedArea = 0;
    uint16_t bottomFixedArea = 0;
    uint16_t scrollHeight = tft.height() - topFixedArea - bottomFixedArea;

    tft.startWrite();
    tft.writecommand(0x33); // VSCRDEF for ST7796S
    tft.writedata(topFixedArea >> 8);
    tft.writedata(topFixedArea & 0xFF);
    tft.writedata(scrollHeight >> 8);
    tft.writedata(scrollHeight & 0xFF);
    ...
    // Now set the scroll start address
    tft.writecommand(0x37); // VSCSAD for ST7796S
    tft.writedata(vsp >> 8);
    tft.writedata(vsp & 0xFF);

    tft.endWrite();
  #elif defined(HAS_ST7789)
    tft.writecommand(ST7789_VSCRDEF); // If using ST7789
    tft.writedata(vsp>>8);
    tft.writedata(vsp & 0xFF);
  #endif
}*/
