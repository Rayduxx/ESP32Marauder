...
void CommandLine::runCommand(String input) {
  ...
  LinkedList<String> cmd_args = this->parseCommand(input, (char*)" ");
    ...
    if (cmd_args.get(0) == ATTACK_CMD) {
      ...
      if (attack_type_switch == -1) {
        ...
        // Beacon
          ...
          Serial.println("Starting  Roll Beacon spam. Stop with " + (String)STOPSCAN_CMD);
          #ifdef HAS_SCREEN
            display_obj.clearScreen();
            menu_function_obj.drawStatusBar();
          #endif
  ...
  // Select access points or stations
    ...
    if (ap_sw != -1) {
      ...
      if (filter_sw != -1) {
        ...
        // Get list of indices
        LinkedList<String> ap_index = this->parseCommand(cmd_args.get(ap_sw + 1), (char*)",");
      ...
      LinkedList<String> sta_index = this->parseCommand(cmd_args.get(cl_sw + 1), (char*)",");
    ...
    // select ssids
      ...
      // Get list of indices
      LinkedList<String> ss_index = this->parseCommand(cmd_args.get(ss_sw + 1), (char*)",");
