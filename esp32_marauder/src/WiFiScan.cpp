...
#include "WiFiScan.h"
#include "lang_var.h"
// Ensure networking/netif headers are available when using esp_netif and the
// esp event loop from multiple WiFi initialization code paths.
#include "esp_netif.h"
#include "esp_event.h"
...
// Provide a local check routine with internal linkage to avoid
// clashing with the esp-idf/libnet80211 implementation which
// also exports ieee80211_raw_frame_sanity_check as an extern C symbol.
// Keep this function local to this translation unit so calls here
// still work but the linker won't see a duplicate global symbol.
static inline int ieee80211_raw_frame_sanity_check(int32_t arg, int32_t arg2, int32_t arg3) {
    if (arg == 31337)
        return 1;
    else
        return 0;
  ...
  class bluetoothScanAllCallback: public NimBLEAdvertisedDeviceCallbacks {
      ...
      void onResult(NimBLEAdvertisedDevice *advertisedDevice) {
        ...
        String display_string = "";
         ...
         if ((wifi_scan_obj.currentScanMode == BT_SCAN_AIRTAG) || (wifi_scan_obj.currentScanMode == BT_SCAN_AIRTAG_MON)) {
...
void WiFiScan::RunSetup() {
  ...
  // Ensure the network interface and default event loop are initialized.
  // Other code paths may call esp_netif_deinit() when shutting down WiFi,
  // so don't permanently remember an "initialized" state -- always try to
  // (re)initialize here and tolerate failures if already initialized.
  {
    esp_err_t _err = esp_netif_init();
    Serial.printf("esp_netif_init() -> 0x%X\n", _err);
    // esp_event_loop_create_default() returns an error if the loop already
    // exists; log the return so the serial trace shows the initialization state.
    _err = esp_event_loop_create_default();
    Serial.printf("esp_event_loop_create_default() -> 0x%X\n", _err);
  }
...
bool WiFiScan::addSSID(String essid) {
  ssid s = {essid, (uint8_t)random(1, 12), {(uint8_t)random(256), (uint8_t)random(256), (uint8_t)random(256), (uint8_t)random(256), (uint8_t)random(256), (uint8_t)random(256)}, false};
  ssids->add(s);
...
void WiFiScan::RunAPScan(uint8_t scan_mode, uint16_t color)
{
  ...
  {
    esp_err_t _err = esp_netif_init();
    Serial.printf("RunAPScan: esp_netif_init() -> 0x%X\n", _err);
    _err = esp_event_loop_create_default();
    Serial.printf("RunAPScan: esp_event_loop_create_default() -> 0x%X\n", _err);
  }
...
void WiFiScan::RunAPInfo(uint16_t index, bool do_display) {
  ...
  if (wps) {
    ...
    if (do_display) {
      ...
      if (wps) {
        display_obj.tft.println("true");
      } else {
        display_obj.tft.println("false");
      }
    }
  #endif
...
void WiFiScan::RunBeaconScan(uint8_t scan_mode, uint16_t color)
{
  if (scan_mode == WIFI_SCAN_AP)
    startPcap("beacon");
      ...
      if (gps_obj.getGpsModuleStatus()) {
        startLog("wardrive");
        String header_line = "WigleWifi-1.4,appRelease=" + (String)MARAUDER_VERSION + ",model=ESP32 Marauder,release=" + (String)MARAUDER_VERSION + ",device=ESP32 Marauder,display=SPI TFT,board=ESP32 Marauder,brand=Raydux\nMAC,SSID,AuthMode,FirstSeen,Channel,RSSI,CurrentLatitude,CurrentLongitude,AltitudeMeters,AccuracyMeters,Type\n";
...
void WiFiScan::apSnifferCallbackFull(void* buf, wifi_promiscuous_pkt_type_t type) {  
  extern WiFiScan wifi_scan_obj;
  // One-time debug print to confirm the sniffer callback is being invoked.
  static bool _apSnifferFirst = false;
  if (!_apSnifferFirst) {
    Serial.println("apSnifferCallbackFull: first packet received (sniffer active)");
    _apSnifferFirst = true;
  }
  wifi_promiscuous_pkt_t *snifferPacket = (wifi_promiscuous_pkt_t*)buf;
  WifiMgmtHdr *frameControl = (WifiMgmtHdr*)snifferPacket->payload;
  wifi_pkt_rx_ctrl_t ctrl = (wifi_pkt_rx_ctrl_t)snifferPacket->rx_ctrl;
  ...
  if (type == WIFI_PKT_MGMT)
  {
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buf == 0))
    {
      ...
      if (!in_list) {
        ...
        AccessPoint ap = {essid,
                          (uint8_t)snifferPacket->rx_ctrl.channel,
                          {snifferPacket->payload[10],
                           snifferPacket->payload[11],
                           snifferPacket->payload[12],
                           snifferPacket->payload[13],
                          ...
                          (int8_t)snifferPacket->rx_ctrl.rssi,
                          new LinkedList<uint16_t>(),
                          0,
                          security_type,
                          wps};
...
void WiFiScan::main(uint32_t currentTime)
{
  ...
  (currentScanMode == WIFI_SCAN_ALL))
  {
    ...
    if (currentTime - initTime >= this->channel_hop_delay * 500) {
      initTime = millis();
      #ifdef HAS_SCREEN
        display_obj.tft.fillRect(0, (STATUS_BAR_WIDTH * 2) + 1 + EXT_BUTTON_WIDTH, TFT_WIDTH, TFT_HEIGHT - STATUS_BAR_WIDTH + 1, TFT_BLACK);
        display_obj.tft.setCursor(0, (STATUS_BAR_WIDTH * 2) + CHAR_WIDTH + EXT_BUTTON_WIDTH);
        display_obj.tft.setTextSize(1);
        display_obj.tft.setTextColor(TFT_WHITE, TFT_BLACK);
  ...
  else if (currentScanMode == WIFI_SCAN_ACTIVE_EAPOL)
  {
    ...
    for (int i = 0; i < access_points->size(); i++) {
      if (access_points->get(i).selected) {
        ssid custom_beacon = {access_points->get(i).essid, (uint8_t)random(1, 12), {(uint8_t)random(256), 
                                                                                      (uint8_t)random(256),
                                                                                      (uint8_t)random(256),
                                                                                      (uint8_t)random(256),
                                                                                      (uint8_t)random(256),
                                                                                      (uint8_t)random(256)}};
        this->broadcastCustomBeacon(currentTime, custom_beacon);
      }
    }
